...
## 2026-05-19 (v4 개발 — PBS, PIBT, 충돌 감지)

### 개요

PBS(Priority-Based Search), PIBT(Priority Inheritance with Backtracking) 알고리즘 추가.
충돌 감지 시스템, 박스 드로잉, UI 개선.
...
- 에이전트를 우선순위 순서대로 순차 계획
- 각 에이전트는 높은 우선순위 에이전트의 전체 경로를 하드 제약으로 사용
- 방식: 전원 도착 → 새 목표 배정 → PBS 재계획 (round-trip)
- CBS보다 빠르지만 완전성·최적성 보장 없음

#### 2. Auto PIBT (`pibt.py`)

PIBT 핵심 동작:
- 매 타임스텝마다 한 칸씩 이동 결정
- 우선순위 높은 로봇이 목표 방향으로 이동 시도
- 목표 칸에 다른 로봇 있으면 그 로봇에게 우선순위 "상속" → 밀어내기
- 사이클 감지 (`in_progress` set)로 무한 재귀 방지

개선 사항 (기본 PIBT 대비):

| 항목 | 기본 PIBT | 우리 구현 |
|---|---|---|
| 방향 결정 | 맨해튼 거리 | **BFS 실제 거리** (벽 우회 가능) |
| Deadlock | 영구 교착 | 8스텝 막히면 **강제 탈출** |
| 우선순위 | 고정 | **동적** (막힌 시간 기반) |

우선순위 결정 기준:
1. `pibt_stuck` 많을수록 → 높은 우선순위 (오래 막힌 로봇 우선)
2. `arrivals` 적을수록 → 높은 우선순위 (공정성)

시각화: 로봇 원 안에 현재 우선순위 순번 표시 (빨간색 = 4스텝 이상 막힘)

#### 3. 충돌 감지 시스템

- 매 프레임 모든 로봇 쌍의 거리 계산 (거리 < 2×radius = 충돌)
- 충돌 로봇에 **빨간 링** 표시
- 하단 stat bar에 충돌 횟수 (충돌 중이면 빨간색으로)
- 좌하단 **Collision Log 패널**: 충돌 내역 누적 표시
- `[L]` 키: 로그 패널 토글, `[F5]`: 카운트 초기화
- 모드 전환/리셋 시 자동 초기화

#### 4. UI 개선

- **박스 드로잉**: Draw/Erase 모드에서 드래그 → 직사각형 범위로 한번에 배치/제거 (반투명 미리보기)
- **스피드 스크롤**: 사이드바에서 마우스 휠 → 속도 조절 (0.5× 단위)
- **섹션 레이블**: 사이드바에 `── Lifelong MAPF ──` / `── MAPF ──` 구분선
- **Lifelong MAPF 버튼 5종**: Auto A\* `[a]`, CBS `[b]`, RHCR `[h]`, PBS `[p]`, PIBT `[i]`

#### 5. ballan 맵 생성

- CSV 파일 (id, l, r, u, d) 형식의 노드 그래프 → PGM 맵 변환
- BFS로 노드 연결 추적 → 2D 그리드 좌표 배치
- 생성된 맵: `maps/ballan.pgm` (53×34 셀, 137개 자유 셀)

---

### 알게 된 것 (v4)

**MAPF vs Lifelong MAPF**
- MAPF: 목표 고정, 전원 도착으로 종료 → SOLVE CBS
- Lifelong MAPF: 도착하면 새 목표 계속 배정, 끝없이 운행 → Auto 모드들

**PBS vs CBS**
- PBS: 우선순위 순서로 순차 계획, 트리 탐색 없음 → 빠름
- CBS: 충돌 트리 전체 탐색 → 최적이지만 느림
- PBS는 우선순위 순서가 잘못되면 해를 못 찾을 수 있음 (불완전)

**PIBT의 한계**
- 한 스텝씩 greedy 결정 → 통로가 좁으면 deadlock 취약
- 완전성 조건: 그래프의 모든 노드가 2개 이상의 이웃 (bi-connected)
- 33x33 벤치마크의 좁은 선반 통로에서는 교착 빈발
- 실제 용도: 넓은 창고 맵에서 100마리+ 대규모 운용

**알고리즘별 적합한 상황**

| 알고리즘 | 에이전트 수 | 맵 특성 | 특징 |
|---|---|---|---|
| CBS | 소수 (5~20) | 제한 없음 | 충돌 완전 보장 |
| RHCR | 중간 (20~50) | 넓은 맵 | 주기적 CBS, 준실시간 |
| PBS | 중간 | 넓은 맵 | 빠름, 완전성 부분 |
| PIBT | 대규모 (100+) | 넓고 우회로 많은 맵 | 초고속, 근사 해 |

---

## 2026-05-19 (v5 개발 — 이종 속도 로봇)

### 개요

에이전트별 이동 속도를 다르게 설정할 수 있는 **이종 속도(heterogeneous speed)** 기능을 CBS에 통합.
`tpc(time_per_cell)` 정수값(1~10)으로 로봇의 칸당 소요 스텝 수를 직접 지정.
UI 전반 키보드 단축키 제거 및 마우스 중심으로 인터페이스 정리.

---

### 구현 내용

#### 1. tpc (time_per_cell) 시스템

**개념**

| tpc | 의미 | 실제 이동 |
|---|---|---|
| 1 | 1 CBS 스텝 = 1칸 이동 | 기본 속도 |
| 2 | 2 CBS 스텝 = 1칸 이동 | 절반 속도 |
| 3 | 3 CBS 스텝 = 1칸 이동 | 1/3 속도 |
| N | N CBS 스텝 = 1칸 이동 | 1/N 속도 |

웨어하우스 로봇 실제 사례: 1.2 m/s 로봇(tpc=1) + 0.7 m/s 로봇(tpc=2) 혼용.

**`astar.py` 수정**

- `time_per_cell` 파라미터 추가 (기본값 1)
- 이동 비용: 대기 = 1 스텝, 이동 = `time_per_cell` 스텝
- 느린 로봇의 transit 중간 스텝에도 vertex constraint 검사:
  ```python
  transit_blocked = any(
...
- `_expand_path(path, tpc)` 함수 추가:
  ```
  tpc=3: [A, B, C] → [A, A, A, B, B, B, C]
  ```
  이동 구간마다 (tpc-1)개의 중복 위치 삽입
- `solve_cbs`에 `time_per_cell` 파라미터 추가
- A\* 결과를 `_expand_path`로 확장 후 CBS 충돌 감지에 사용
- 각 에이전트 독립적인 tpc 지원

**시각적 속도 표현**

- CBS/RHCR/PBS 모드: `local_t`는 모든 에이전트 동일 속도로 증가
  - 느린 로봇의 확장 경로가 길어서 자연스럽게 시각적 차이 표현
  - tpc=3 로봇: path[0~2]=A → local_t 0~2 동안 A에 머뭄 → B로 이동
- Auto A\* 모드: `local_t += dt * sim_speed / tpc` (직접 감속)

---

#### 2. RHCR H ≥ max(tpc) 제약

**문제**: H < tpc이면 느린 로봇이 H 스텝 안에 한 칸도 못 가고 재계획 → 제자리 루프.

**규칙**: H는 항상 `max(모든 에이전트의 tpc)` 이상이어야 함.

| 상황 | 동작 |
|---|---|
| tpc 버튼 클릭으로 tpc 증가 | H 자동으로 max(tpc)까지 올림, 팝업에 `(H→N)` 표시 |
| UI H `[-]` 클릭 | H가 max(tpc) 아래로 내려가지 않도록 클램프 |
...
- 각 에이전트 행 오른쪽: tpc 값 + `(스텝/칸)` 단위 표시
  - 숫자: Consolas, 단위: Malgun Gothic (한글 지원)
- 좌클릭: tpc -1 (빠르게), 우클릭: tpc +1 (느리게)
- 변경 시 `_cont_agents` 동시 업데이트 (RHCR 즉시 반영)

**RHCR W/H 사이드바 컨트롤**

- `Auto: RHCR` 버튼 아래 `RHCR W: N [-][+]` / `RHCR H: N [-][+]` 행 추가
- W: ±1씩 조절 (최소 H, 최대 200)
- H: ±1씩 조절 (최소 max(tpc), 최대 W)

**Random Agents 속도 설정**

- 에이전트 수 입력 후 두 번째 다이얼로그:
  - 숫자(1~10) 입력: 전체 동일 tpc
  - `r` 또는 빈칸: 각 에이전트 1~10 랜덤 tpc

**키보드 단축키 전면 제거**

- 모든 조작을 사이드바 버튼 + 마우스로만 수행
- Radius: 바 좌클릭 +0.5, 우클릭 -0.5
- Speed: 사이드바 스크롤 휠 유지

**창 위치**

- `SDL_VIDEO_CENTERED=1`: 실행 시 모니터 중앙에 창 표시
...
| 버그 | 원인 | 해결 |
|---|---|---|
| RHCR에서 tpc 변경 무효 | 클릭 핸들러가 `agents["tpc"]`만 업데이트, `_cont_agents["tpc"]`는 갱신 안 함 | 클릭 시 `_cont_agents`도 동시 업데이트 |
| CBS/RHCR에서 느린 로봇 4배 감속 | 경로 확장 + `local_t`에 speed 배수까지 이중 적용 | CBS 모드에서 `local_t` speed 배수 제거 (경로 확장만 사용) |
| 팝업이 구 speed(x0.2) 표시 | 클릭 핸들러가 구형 `speed` float 코드로 남아있음 | 핸들러 전체를 tpc 정수 방식으로 교체 |
| H < tpc 시 제자리 루프 | H CBS 스텝 안에 tpc 스텝이 안 채워져 로봇 미이동 | H min 클램프를 max(agent tpc)로 설정 |
...
- CBS의 충돌 감지는 CBS 타임스텝 기준으로 동작
- tpc=3 로봇이 A→B 이동 중 CBS 타임스텝 1, 2에서는 "A에 있음"으로 처리
- 확장 경로 덕분에 tpc가 다른 로봇 간 충돌도 정확히 감지·해결 가능

**H와 tpc의 관계**

- H는 CBS 타임스텝 단위
- tpc=3 로봇은 H 스텝 동안 H/3 칸 이동 → H ≥ tpc 이어야 최소 1칸 보장
- W는 tpc에 무관하게 CBS 타임스텝으로 표현 (W/tpc = 계획된 물리적 칸 수)
